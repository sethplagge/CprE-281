module regfile(DATAP3, DATAP2, DATAP1, DATAP0, DATAQ3, DATAQ2, DATAQ1, DATAQ0, RP2, RP1, RP0, RQ2, RQ1, RQ0, WA2, WA1, WA0, LD_DATA, WR, CLRN, CLK);
	...
	reg4b my_reg0(.IN(LD_DATA), .LD(Y0), .CLK(CLK), .OUT(VALUE0), .CLRN(CLRN));
	reg4b my_reg1(.IN(LD_DATA), .LD(Y1), .CLK(CLK), .OUT(VALUE1), .CLRN(CLRN));
	reg4b my_reg2(.IN(LD_DATA), .LD(Y2), .CLK(CLK), .OUT(VALUE2), .CLRN(CLRN));
	reg4b my_reg3(.IN(LD_DATA), .LD(Y3), .CLK(CLK), .OUT(VALUE3), .CLRN(CLRN));
	reg4b my_reg4(.IN(LD_DATA), .LD(Y4), .CLK(CLK), .OUT(VALUE4), .CLRN(CLRN));
	reg4b my_reg5(.IN(LD_DATA), .LD(Y5), .CLK(CLK), .OUT(VALUE5), .CLRN(CLRN));
	reg4b my_reg6(.IN(LD_DATA), .LD(Y6), .CLK(CLK), .OUT(VALUE6), .CLRN(CLRN));
	reg4b my_reg7(.IN(LD_DATA), .LD(Y7), .CLK(CLK), .OUT(VALUE7), .CLRN(CLRN));
